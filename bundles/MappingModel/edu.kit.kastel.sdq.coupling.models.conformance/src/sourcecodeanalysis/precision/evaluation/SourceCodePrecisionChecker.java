...
public class SourceCodePrecisionChecker {
    ...
    /**
     * Evaluates precision and recall by comparing observed flows against the ground truth.
     */
    public ComparisonResult evaluate(List<ObservedFlow> observed, Set<GroundTruthFlow> groundTruth) {
        ...
        for (ObservedFlow o : observed) {
            ...
            Optional<GroundTruthFlow> match = unmatchedGT.stream()
                    .filter(gt -> matches(o, gt))
                    .findFirst();

            if (match.isPresent()) {
                tp++;
                unmatchedGT.remove(match.get());  // Remove matched GT so it's not counted twice
            } else {
                fp++;
            }
        }

        int fn = unmatchedGT.size(); // Any unmatched GT flows are false negatives
        return new ComparisonResult(tp, fp, fn);
    }
    
    /**
     * Evaluates precision and recall by comparing observed flows against the ground truth,
     * and returns true if precision > 0.5 and recall >= 1.
     */
    public boolean isAcceptablePrecisionRecall(List<ObservedFlow> observed, Set<GroundTruthFlow> groundTruth) {
        ...
        return precision > 0.5 && recall >= 1.0;
    }
     ...
     * Source component must match
     * Target component must match
     * security levels must match
     */
    private boolean matches(ObservedFlow o, GroundTruthFlow gt) {

        if (!o.dataType().equals(gt.dataType())) return false;

        if (!o.fromComponent().equals(gt.fromComponent())) return false;

        if (!o.toComponent().equals(gt.toComponent())) return false;

        if (!o.fromLevel().equals(gt.fromLevel())) return false;
